...
class ViewRefundRequest extends ViewRecord
{
    ...
    public function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                Infolists\Components\Section::make('Refund Request Details')
                    ->schema([
                        ...
                        Infolists\Components\TextEntry::make('order.id')
                            ->label('Order ID')
                            ->url(fn($record) => "/admin/orders/{$record->order->id}"),


                        Infolists\Components\TextEntry::make('user.name')
                            ->label('Requested By'),

                        Infolists\Components\TextEntry::make('approve')
                            ->label('Status')
                            ->badge()
                ...
                Infolists\Components\Section::make('Order Information')
                    ->schema([
                        Infolists\Components\TextEntry::make('order.total.formatted')
                            ->label('Order Total'),
                        ...
                        Infolists\Components\TextEntry::make('availableToRefund')
                            ->label('Available to Refund')
                            ->money(fn() => $this->record->order->currency->code)
                            ->getStateUsing(fn() => $this->availableToRefund / $this->record->order->currency->factor),
                    ])
                    ->columns(3)
                    ->visible(fn() => $this->record->order),

                Infolists\Components\Section::make('Comments & Notes')
                    ->schema([
